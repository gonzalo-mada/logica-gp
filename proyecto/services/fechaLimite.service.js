...
function getFechaLimite(request, response) {
    var dateFormat = require('dateformat');
    try {
        var args = JSON.parse(request.body.arg === undefined ? "{}" : request.body.arg);
        if (args.hasOwnProperty("codigoPrograma")) {
            var con = new sql.Connection(global.config.ds_PSMDB, function (err) {
                var stmt = new sql.Request(con);
                stmt.input("codigoPrograma", sql.BigInt, args.codigoPrograma);
                stmt.execute("BAS_SEL_FechaLimite").then(function (recordsets) {
